...
from PySide6.QtCore import Qt, QSettings, Signal, QObject, QByteArray, QEvent
...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QPlainTextEdit, QProgressBar, QSizePolicy, QDialog, QMenu
)
...
from dictionaries.loader import load_dictionary, get_synonyms
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        v = QVBoxLayout(self)
        self.btn_load = QPushButton("Подгрузить")
    ...
    def _load(self):
        ...
        load_dictionary(progress)
        QMessageBox.information(self, "Словари", "Словари загружены")
...
class EditorArea(QFrame):
    def __init__(self):
        ...
        self.tran.installEventFilter(self)
    ...
    def eventFilter(self, obj, event):
        if obj is self.tran and event.type() == QEvent.MouseButtonDblClick:
            cursor = self.tran.cursorForPosition(event.pos())
            cursor.select(QTextCursor.WordUnderCursor)
            word = cursor.selectedText().strip()
            if word:
                syns = get_synonyms(word)
                if syns:
                    menu = QMenu(self.tran)
                    cur_copy = QTextCursor(cursor)
                    for s in syns:
                        act = menu.addAction(s)
                        act.triggered.connect(lambda chk=False, c=QTextCursor(cur_copy), o=word, s=s: self._apply_synonym(c, o, s))
                    menu.exec(self.tran.mapToGlobal(event.pos()))
            return True
        return super().eventFilter(obj, event)

    def _apply_synonym(self, cursor: QTextCursor, original: str, synonym: str):
        cursor.beginEditBlock()
        cursor.removeSelectedText()
        cursor.insertText(synonym)
        cursor.endEditBlock()
        self.tran.setTextCursor(cursor)
        if QMessageBox.question(self, "Глоссарий", f"Добавить '{original}' → '{synonym}' в глоссарий?") == QMessageBox.Yes:
            base = ensure_dir(self.project_path or Path.cwd())
            gloss = base / "glossary.txt"
            with open(gloss, "a", encoding="utf-8") as f:
                f.write(f"{original}\t{synonym}\n")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.btn_settings = QPushButton("⚙"); self.btn_settings.setFixedWidth(32); self.btn_settings.clicked.connect(self._open_settings)
        ...
        top_bar.addWidget(self.btn_burger); top_bar.addWidget(title); top_bar.addStretch(1); top_bar.addWidget(self.btn_settings); top_bar.addWidget(self.btn_fullscreen)
