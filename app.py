...
from translators import create_translator, Translator
...
class ProjectStore:
    def __init__(self, workdir: Path):
        ...
        # Migrate old DBs without model column
        cur.execute("PRAGMA table_info(projects)")
        cols = [r[1] for r in cur.fetchall()]
        if "model" not in cols:
            cur.execute("ALTER TABLE projects ADD COLUMN model TEXT NOT NULL DEFAULT 'gemini'")
    ...
    def list(self, status="active"):
        ...
        conn = sqlite3.connect(self.db); cur = conn.cursor()
        cur.execute(
            "SELECT id,name,model,status,created_at,updated_at FROM projects WHERE status=? ORDER BY id DESC",
            (status,),
        )
        rows = cur.fetchall(); conn.close(); return rows

    def create(self, name: str, model: str = "gemini"):
        import sqlite3
        now = datetime.utcnow().isoformat()
        conn = sqlite3.connect(self.db); cur = conn.cursor()
        cur.execute(
            "INSERT INTO projects(name,model,status,created_at,updated_at) VALUES(?, ?, 'active', ?, ?)",
            (name, model, now, now),
        )
...
class ProjectPanel(QFrame):
    ...
    def _add(self):
        ...
        name_edit = QLineEdit()
        model_box = QComboBox()
        model_box.addItems(["gemini", "gpt5", "claude", "grok"])
        form.addRow("Название:", name_edit)
        form.addRow("Модель:", model_box)
        buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        buttons.accepted.connect(dlg.accept)
        buttons.rejected.connect(dlg.reject)
        form.addWidget(buttons)
        if dlg.exec() and name_edit.text().strip():
            self.store.create(name_edit.text().strip(), model_box.currentText())
            self.refresh()
...
class EditorArea(QFrame):
    ...
    def bind_project(self, project_path: Path, translator: Translator | None = None):
...
class MainWindow(QMainWindow):
    ...
    def _bind_project(self, pid: int):
        ...
        proj = next((r for r in rows if r[0] == pid), None)
        if proj:
            name = proj[1]
            model = proj[2] or "gemini"
            key_map = {
            ...
            translator = create_translator(model, api_key=key_map.get(model))
            self.editor.bind_project(Path(self.workdir) / name, translator)
